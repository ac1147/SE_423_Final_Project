...
#define OFFNOTE 1
#define EYE_OF_THE_TIGER 13

//Notes for song
uint16_t songArray[EYE_OF_THE_TIGER] = {
                                        F4NOTE, G4NOTE, G4SHARPNOTE, G4SHARPNOTE, G4SHARPNOTE, G4SHARPNOTE, G4NOTE, F4NOTE, D4NOTE, D4NOTE, F4NOTE, G4NOTE, F4NOTE
};

//wait time in ms for in between notes
uint32_t durationArray[EYE_OF_THE_TIGER] = {
                                            2000, 2000, 2000, 2000, 2000, 2000, 1000, 2000, 2000, 2000, 2000, 20000, 2000
};
...
////Obstacle Detection
//float x_predo[2][1] = {{0},{0}};                 // predicted state
...
#define rightTongueAngle -28
#define leftTongueAngle 56
...
uint16_t c24count = 0;
uint16_t c26count = 0;
uint16_t c50count = 0; //AC counter for case 50
uint16_t c32count = 0;
uint16_t c34count = 0;
uint16_t c36count = 0;
uint16_t c40count = 0;
...
float kpvision = -0.05;
int16_t orbl_count = 0;
int16_t prbl_count = 0;

//variable for cputimer turn on

uint16_t play_song = 0;

// variable for timer
uint16_t song_count = 0;

uint16_t times_looped = 0;
...
void main(void)
{
    ...
    ConfigCpuTimer(&CpuTimer2, LAUNCHPAD_CPU_FREQUENCY, 400000); // Currently not used for any purpose
    ...
    EPwm9Regs.TBCTL.bit.CTRMODE = 0;    // JB: Count-Up Mode
    EPwm9Regs.TBCTL.bit.FREE_SOFT = 2;    // JB: Free Run mode (PWM continues during breakpoints)
    EPwm9Regs.TBCTL.bit.PHSEN = 0;        // JB: Disable phase loading
    EPwm9Regs.TBCTL.bit.CLKDIV = 1;       // JB: Clock divide set to 1 (50MHz clock)
    EPwm9Regs.TBCTR = 0;                // JB: Start counter at zero
    EPwm9Regs.TBPRD = 10000;            // JB: Set period for 5kHz PWM (200us period)
    //EPwm9Regs.CMPA.bit.CMPA = 5000;       // JB: Initialize duty cycle at 50%
    EPwm9Regs.AQCTLA.bit.CAU = 0;    // JB: What to do when CMPA is reached
    EPwm9Regs.AQCTLA.bit.ZRO = 3;    // JB: What to do when CNT set back to zero
    EPwm9Regs.TBPHS.bit.TBPHS = 0;        // JB: Zero phase shift


    //    GPIO_SetupPinMux(16, GPIO_MUX_CPU1, 5); // Set as GPIO66 and used as MPU-9250 SS
    //    GPIO_SetupPinOptions(16, GPIO_OUTPUT, GPIO_PUSHPULL); // Make GPIO66 an Output Pin
    ...
    robotdest[8].x = 5;     robotdest[8].y = 5;
    ...
    while(1)
    {
        if (UARTPrint == 1 ) {
            ...
            if (readbuttons() == 0) {
                // UART_printfLine(2,"Ang:%.2f",RCangle);
                //                //                UART_printfLine(1,"x:%.2f:y:%.2f:a%.2f",ROBOTps.x,ROBOTps.y,ROBOTps.theta);
                //                UART_printfLine(2,"F%.4f R%.4f",LADARfront,LADARrightfront);
                //Lab7 Part Distance Value Reading

                // AC test potential Lidar values for detect legs
                //                UART_printfLine(1,"%.0f,%.1f,%.1f,%.1f",tagid,tagx,tagy,tagz);
                //                UART_printfLine(2,"%.1f,%.1f,%.1f",tagthetax,tagthetay,tagthetaz);
                //                UART_printfLine(1, "LADAR 100:%.2f 110:%.2f", ladar_data[100].distance_ping, ladar_data[110].distance_ping);


                UART_printfLine(1, "state:%.d", RobotState);
                UART_printfLine(2, "tagid:%.3f", tagid);
                //                UART_printfLine(2, "id: %.3f", tagid);
...
__interrupt void cpu_timer2_isr(void)
{
    ...
    //    ConfigCpuTimer(&CpuTimer2, LAUNCHPAD_CPU_FREQUENCY, durationArray[song_index] * 1000);

    song_index++;

    if(song_index >= EYE_OF_THE_TIGER) {
        song_index = 0;
    }
...
__interrupt void SWI1_HighestPriority(void)     // EMIF_ERROR
{
    ...
    dist = (p1*pow(MaxRowThreshold1, 3) + (p2*pow(MaxRowThreshold1, 2)) + (p3*MaxRowThreshold1) + p4);
    ...
    if(calibration_state == 0){
        ...
        //        //Obstacle Detection
        //        Matrix2x1_Add(x_predo, Bu, x_predo, 1.0, 1.0);
        ...
        case 1:
            ...
            if ((MaxRowThreshold1 > 30)&&(c1count > 2000)) {
                RobotState = 20;
            }
            if ((MaxRowThreshold2 > 30)&&(c1count > 2000)) {
                RobotState = 30;
            }
            if((tagid == 5.0 ) &&(ballCount ==4)){
                //go home feature
                //change it to the actual value for the april tag
                statePos = 0;
                RobotState = 50;
            }
        ...
        case 20: //---------GREEN---------
            ...
            //Lab7 Part2 update colthreshold

            // AC set colcentriod as 0 when ball is at center left to right
            colcentroid = MaxColThreshold1 - 80;

            // AC if green ball is close enough to robot
            if ((MaxColThreshold1 == 0) || (MaxAreaThreshold1 < 3)){
                // AC stop robot
                //                vref = 0;
                //                turn = 0;
            } else {
                // AC if orange ball is close enough to robot to robot and centered enough (hehe)  && (fabs(0 - colcentroid) < 5)
                if ((MaxRowThreshold1 > 89) && (fabs(0 - colcentroid) < 5)) {
                    // AC go to state 22 and reset counter
                    RobotState = 22;
                    c22count= 0;
                    vref = 0;
                    turn = 0;
        ...
        case 22:  //---------GREEN---------
            // AC State 22 stops robot and opens gripper door opening for orange ball
            ...
            // AC move tongue left for orange ball after 0.5s
            ...
            if (c22count>500)
            {
                setEPWM6A_RCServo(rightTongueAngle);
            }

            // AC open door after 2s
            if (c22count>2000)
            {
                setEPWM5B_RCServo(-80);
                RobotState = 24;
                c24count = 0;
            }
            ...
            // AC open door
            //            setEPWM5B_RCServo(-80);
            // AC move tongue right for orange
            //            setEPWM6A_RCServo(rightTongueAngle);

            break;
        case 24: //---------GREEN---------
            // AC state 24 drives forward slowly over orange ball
            // AC speed slow
            vref = 0.5;
            turn = 0;
            // AC reset counter
            ...
            break;
        case 26:  //---------GREEN---------
            // AC state 26 stop robot and catch the orange ball
            ...
            // AC close gripper
            setEPWM5B_RCServo(13);

            //why
            setEPWM6A_RCServo(56);


            // AC after 1 second
            if(c26count >1000){

                // AC return to state 1 and continue on XY path
                RobotState = 1;
                // AC reset counter
                c1count = 0;
                if(times_looped ==0){
            ...
            //home if two orange balls collected
            //            if (orbl_count >= 2){
            //                play_song = 1;
            //                ballCount = 0;
            //                RobotState = 40;
        ...
        case 30:  //---------ORANGE---------
            // put vision code here: collect

            //Lab7 Part2 update colthreshold
            colcentroid = MaxColThreshold2 - 80;

            // AC if orange ball is close enough to robot
            ...
            // AC if orange ball close enough to robot && (fabs(0 - colcentroid) < 20)
            if ((MaxRowThreshold2 > 89)  && (fabs(0 - colcentroid) < 5)) {
                // AC go to state 32 and reset counter
                RobotState = 32;
                c32count= 0;
                vref = 0;
                turn = 0;
        ...
        case 32: //---------ORANGE---------
            // AC State 32 stops robot and opens gripper door opening for purple ball
            ...
            if (c32count>500)
            {
                setEPWM6A_RCServo(leftTongueAngle);
            }
            ...
            break;
        case 34: //---------ORANGE---------
            // AC state 34 drives forward slowly over orange ball
            // AC speed slow
            vref = 0.5;
            turn = 0;
            c34count++;
            ...
            break;
        case 36: //---------ORANGE---------
            // AC state 36 stop robot and catch the orange ball
        ...
        case 40:  //---------HOME---------
            ...
            robotdest[statePos].x = 0;
            robotdest[statePos].y = 0;
            //robotdest[statePos].theta = 0;

            break;

        case 50: //---------drop off ORANGE balls---------

            robotdest[statePos].x = -4;
            robotdest[statePos].y = 2;
            robotdest[statePos].theta = -90;
            c50count ++;
            if(tagid == 6.0){
                c51count =0;
                RobotState = 51;
            ...
            break;
        case 51:
            // AC stop, move tongue left, open gripper
        ...
        case 60: //---------drop off GREEN balls---------
            ...
            break;
        case 61:
            // AC stop, move tongue right, open gripper
            vref = 0;
            turn = 0;
            c61count++;
            if (c61count>500)
        ...
        case 62:
            ...
            //move backwards to release balls
            vref = -0.5;
            turn = 0;
            c62count++;
            ...
            //        case 50:
            //          // AC drop balls off at desired location
            //
            //          // AC Go back to start
            //          statePos = 0;
            //
            //          // AC if object seen right wall follow
            //          if (LADARfront < 1.2) {
            //                vref = 0.2;
            //                checkfronttally++;
            //                if (checkfronttally > 310) { // check if LADARfront < 1.2 for 310ms or 3 LADAR samples
            ...
            //            }
            //
            //          // AC if close to start
            //          if (xy_control(&vref, &turn, 1.0, ROBOTps.x, ROBOTps.y, robotdest[statePos].x, robotdest[statePos].y, ROBOTps.theta, 0.25, 0.5)) {
            //            // AC open door
            //            vref = 0;
            //            turn = 0;
            //            setEPWM5B_RCServo(-80);
            //            // AC move tongue to neutral
            //            setEPWM6A_RCServo(14);
            //
            //            // start counter
            //            ++c50count;
            //            if (c50count >1000) {
            //              // AC blind drive foward to loose balls
            //              vref = 0.5;
            //              turn = 0;
            //            }
            //
            //            if (c50count >2000) {
            //              // AC stop robot job is done
            //              vref = 0;
            //              turn = 0;
            //            }
            //
        ...
        if((timecount%250) == 0) {
            ...
            //            DataToLabView.floatData[3] = xo;
            //            DataToLabView.floatData[4] = yo;
